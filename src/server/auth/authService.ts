...
export async function findUser(username: string) { const result = await pool.query("SELECT id, username, password_hash, role, is_active, must_change_password, last_login_at FROM app_users WHERE username = $1 AND deleted_at IS NULL", [normalize(username)]); const row = result.rows[0]; return row ? normalizeAuthUserRow(row) as (AuthUser & { password_hash: string; is_active: boolean; last_login_at?: string | null }) : undefined; }
export async function createSession(userId: number, req: Request) { const token = randomBytes(32).toString("hex"); await pool.query("INSERT INTO app_user_sessions (user_id, token_hash, expires_at, user_agent, ip_address) VALUES ($1, $2, LEAST(now() + make_interval(hours => $3), now() + make_interval(hours => $4)), $5, $6)", [userId, tokenHash(token), config.auth.sessionIdleHours, config.auth.sessionMaxLifetimeHours, req.get("user-agent") || null, req.ip]); return token; }
export async function userFromToken(token: string | undefined) { if (!token) return undefined; const result = await pool.query(`SELECT u.id, u.username, u.role, u.must_change_password AS "mustChangePassword", u.last_login_at AS "lastLoginAt" FROM app_user_sessions s JOIN app_users u ON u.id = s.user_id WHERE s.token_hash = $1 AND s.revoked_at IS NULL AND s.expires_at > now() AND u.is_active = true AND u.deleted_at IS NULL`, [tokenHash(token)]); const row = result.rows[0]; return row ? normalizeAuthUserRow(row) : undefined; }
export async function refreshSession(token: string | undefined) { if (!token) return undefined; const result = await pool.query(`UPDATE app_user_sessions s SET last_seen_at = now(), expires_at = LEAST(now() + make_interval(hours => $2), s.created_at + make_interval(hours => $3)) FROM app_users u WHERE s.user_id = u.id AND s.token_hash = $1 AND s.revoked_at IS NULL AND s.expires_at > now() AND u.is_active = true AND u.deleted_at IS NULL RETURNING s.expires_at`, [tokenHash(token), config.auth.sessionIdleHours, config.auth.sessionMaxLifetimeHours]); return result.rows[0]?.expires_at as Date | undefined; }
