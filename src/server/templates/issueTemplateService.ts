...
export async function buildIssueTemplatePreview(issueId: number, kind: IssueTemplateKind) {
  ...
  return {
    ...
    templatePath,
    ...renderTemplateParagraphs(paragraphs, values)
  };
}
...
function renderTemplateParagraphs(paragraphs: TemplateParagraph[], values: {
  ...
  for (const paragraph of paragraphs) {
    if (/<<\s*(judul|item) object/i.test(paragraph.text)) {
      if (!objectListInserted) {
        ...
        renderedHtml.push(formatTemplateObjectListHtml(values.objectList));
  ...
  return {
    body: rendered.join("\n").replace(/\n{3,}/g, "\n\n").trim(),
    bodyHtml: renderedHtml.join("").replace(/(?:<p><\/p>){3,}/g, "<p></p><p></p>").trim()
  };
}
...
function readDocxParagraphs(filePath: string) {
  ...
  return paragraphs
    .map((paragraph) => extractParagraphRich(paragraph));
}
...
function extractParagraphRich(paragraphXml: string): TemplateParagraph {
  ...
  const listMeta = extractListMeta(paragraphXml);
  return {
    text: fallbackText,
...
function extractListMeta(paragraphXml: string) {
...
function wrapTemplateParagraphHtml(html: string, paragraph: TemplateParagraph, plainText: string) {
  ...
  const ordered = paragraph.listId && paragraph.listId !== "3";
  return `<div class="template-list-line ${ordered ? "ordered" : "bulleted"} level-${Math.max(0, paragraph.listLevel)}">${content}</div>`;
}
...
function formatTemplateObjectListHtml(objectList: string) {
  ...
  return `<ul class="template-object-list">${groups.map((group) => {
    const [label, ...items] = group.split("\n").map((line) => line.trim()).filter(Boolean);
    return `<li><span>${escapeHtml(label || "Object")}</span>${items.length ? `<ul>${items.map((item) => `<li><strong>${escapeHtml(item)}</strong></li>`).join("")}</ul>` : ""}</li>`;
  }).join("")}</ul>`;
}
