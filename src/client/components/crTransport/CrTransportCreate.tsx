...
export function getResolvedObjectHint(query: string, item: ResolvedTransportObject) {
  const searched = String(query || "").trim().toUpperCase();
  const resolved = String(item.objectName || "").trim().toUpperCase();
  if (!searched || searched === resolved || String(item.objectType || "").trim().toUpperCase() !== "FUGR") return "";
  return `${searched} resolves to transport root ${resolved} (Function Group).`;
}
