...
All recipients are sent as Multiple To. There is no CC/BCC behavior in this release.

## MCP Email Integration

The configured MCP server exposes `send_email` with `to`, `subject`, and `body` string parameters. The body uses plain text/Markdown.

- The app constructs one `to` string by joining the de-duplicated recipient addresses with commas.
- Before production use, a controlled mailbox test must verify that this MCP implementation accepts comma-separated Multiple To recipients.
- The production send endpoint must reject the request if the Multiple To format is not enabled/verified by configuration; it must not silently send one email per recipient.
- The endpoint records the MCP response or failure without logging secrets.
...
- Disable submission while preview or send is in progress. Refresh Issue detail/list state after a successful send.
...
- Perform one explicitly authorized controlled send to a test mailbox to verify comma-separated Multiple To behavior before enabling production sending.
